...
- [오해들](#the-misconceptions)
    - [1) `T`는 owned types만을 포함합니다.](#1-t-only-contains-owned-types)
    - [2) `T: 'static`라면 `T`는 프로그램 전체에서 유효해야만 합니다.](#2-if-t-static-then-t-must-be-valid-for-the-entire-program)
    - [3) `&'a T`와 `T: 'a`는 같습니다.](#3-a-t-and-t-a-are-the-same-thing)
    - [4) 제 코드는 제너릭도 아니고, 라이프타임도 가지지 않습니다.](#4-my-code-isnt-generic-and-doesnt-have-lifetimes)
    - [5) 컴파일이 잘 된다면 라이프타임 어노테이션들이 정확하게 맞아떨어진겁니다.](#5-if-it-compiles-then-my-lifetime-annotations-are-correct)
    - [6) boxed 트레잇 객체들은 라이프타임을 갖지 않습니다.](#6-boxed-trait-objects-dont-have-lifetimes)
    - [7) 컴파일러 에러 메세지는 내 프로그램을 고칠 방법을 알려줄 겁니다.](#7-compiler-error-messages-will-tell-me-how-to-fix-my-program)
    - [8) 라이프타임은 런타임에 늘어나거나 줄어들 수 있습니다.](#8-lifetimes-can-grow-and-shrink-at-run-time)
    - [9) mut refs에서 shared refs로 다운그레이드하는건 안전합니다.](#9-downgrading-mut-refs-to-shared-refs-is-safe)
    - [10) closures는 함수처럼 같은 라이프타임 elision 룰을 따릅니다.](#10-closures-follow-the-same-lifetime-elision-rules-as-functions)
    - [11) `'static` refs는 항상  can always be coerced into `'a` refs](#11-static-refs-can-always-be-coerced-into-a-refs)
